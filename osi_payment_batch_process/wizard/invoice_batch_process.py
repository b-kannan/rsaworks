...
class InvoiceCustomerPaymentLine(models.TransientModel):
    ...
    def _onchange_amount(self):
        check_amount_in_words = num2words(
                            math.floor(self.receiving_amt),
                            lang='en'
                            ).title()
        decimals = self.receiving_amt % 1
        if decimals >= 10 ** -2:
            check_amount_in_words += _(' and %s/100') % str(int(round(\
...
class InvoicePaymentLine(models.TransientModel):
    ...
    def _onchange_amount(self):
        check_amount_in_words = num2words(math.floor(self.paying_amt),
                                          lang='en').title()
...
class AccountRegisterPayments(models.TransientModel):
    ...
    def _compute_customer_pay_total(self):
        self.total_customer_pay_amount = sum(line.receiving_amt for\
                                line in self.invoice_customer_payments)

    @api.multi
    @api.depends('invoice_payments.paying_amt')
    def _compute_pay_total(self):
        self.total_pay_amount = sum(line.paying_amt for\
                             line in self.invoice_payments)

    is_auto_fill = fields.Char(string="Auto-Fill Pay Amount")
    invoice_payments = fields.One2many('invoice.payment.line',
                                       'wizard_id',
                                       string='Payments')
    ...
    def get_payment_batch_vals(self, inv_payment=False, group_data={}):
        if group_data:
            res = {
                ...
                'payment_method_id': 'payment_method_id' in group_data and\
                 group_data['payment_method_id'] or self.payment_method_id.id,
            ...
            if self.payment_method_id == self.env.ref(\
                ...
                res.update({
                    ...
                    group_data['total_check_amount_in_words'] or '',
                })
            return res
    ...
    def make_payments(self):
        ...
        if self.is_customer:
            ...
            for data_get in self.invoice_customer_payments:
                if data_get.receiving_amt > 0:
                    data_get.payment_difference = data_get.balance_amt - \
                    data_get.receiving_amt
                    ...
                    if partner_id in group_data:
                        ...
                        group_data[partner_id].update(\
                          {'partner_id': partner_id,
                           ...
                           'payment_method_id': data_get.payment_method_id and\
                            data_get.payment_method_id.id or False,
                        ...
                        group_data[partner_id]['inv_val'].update(
                            ...
                            data_get.writeoff_account_id.id or False,
                            }})
                    else:
                        # build memo value
        ...
        for p in list(group_data):
            ...
            if context.get('active_ids',False) and \
                group_data[p].get('temp_invoice', False):
