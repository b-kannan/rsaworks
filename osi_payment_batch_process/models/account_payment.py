...
class AccountPayment(models.Model):
    ...
    def _create_payment_entry(self, amount):
        ...
        if 'group_data' in self._context:
            ...
            if self.invoice_ids and all([x.currency_id == self.invoice_ids[0].\
                                    currency_id for x in self.invoice_ids]):
            ...
            for inv in self._context.get('group_data')[p_id]['inv_val']:
                ...
                self._context.get('is_customer'):
                    ...
                    amt = self._context.get('group_data')[p_id]['inv_val'][inv]['paying_amt']
                debit, credit, amount_currency, currency_id = aml_obj.\
                with_context(date=self.payment_date)._compute_amount_fields(
                ...
                counterpart_aml_dict = self._get_shared_move_line_vals(\
                                            ...
                                            move.id, currunt_invoice)

                counterpart_aml_dict.update(self.\
                            _get_counterpart_move_line_vals(currunt_invoice))
